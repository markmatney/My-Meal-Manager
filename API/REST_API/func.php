...
class databaseAPI {
	...
	*/
	...
	Takes in username and returns true if user exists,
	false otherwise.
	*/
	...
	*/
	function getInventory($userid){
		...
		while($row = @mysql_fetch_assoc($invenlist)) 
		{
			...
			$qty = $row['Quantity'];
			$unit = $row['Units'];
			$array2 = array('name' => $name, 'quantity' => $qty, 'units' => $unit);
			$array[$i] = $array2;
			$i = $i+1;
			
		}
		return json_encode(array('items' => $array));
	}
	...
	*/
	function getRecipeID($recipe){
	     $id =  mysql_fetch_assoc(@mysql_query("SELECT RecipeID FROM Recipes WHERE RecipeName = '$recipe'"));
	...
	function getRecipeIngredients($recipe){
	   $recipeid = $this->getRecipeID($recipe);
		...
		$i = 0;
		   ...
		   $qty = $row['Quantity'];
		   $unit = $row['Units'];
		     $array2 = array('name' => $name, 'units' => $unit, 'quantity' => $qty);
			$array[$i] = $array2;
			$i = $i+1;
	...
	*/
	function getRecipe($recipe){
			...
			//$ingredients = "";
			 ...
			 $array = array('recipe_name' => $name, 'url' => $url, 'image_url' => $img, 'time' => $time, 'ingredients' => $ingredients, 'instructions' => $instructions);
	...
	*/
	function getUserRecipes($userid){
	  ...
	  while($row = mysql_fetch_assoc($list)) 
	   ...
	   return json_encode(array('recipes' => $array));
	}
	...
	*/
	...
	function searchRecipes($userid, $ingreds){
		...
		echo json_encode(array("recipes" => $results));
	}
}
?>
